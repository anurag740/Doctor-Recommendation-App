...
if "RENDER" in os.environ:
    logging.info("Installing Chrome & ChromeDriver on Render...")
    ...
    subprocess.run("curl -Lo /tmp/chromedriver.zip https://storage.googleapis.com/chrome-for-testing-public/122.0.6261.111/linux64/chromedriver-linux64.zip", shell=True, check=True)
    subprocess.run("unzip /tmp/chromedriver.zip -d /tmp", shell=True, check=True)
    subprocess.run("sudo mv /tmp/chromedriver-linux64/chromedriver /usr/bin/chromedriver", shell=True, check=True)
    subprocess.run("sudo chmod +x /usr/bin/chromedriver", shell=True, check=True)
...
# Function to scrape Google search results for doctors
def scrape_doctors(specialty, location):
    ...
    if not os.path.exists(chromedriver_path):
        logging.error("ChromeDriver not found at: " + chromedriver_path)
        return []
    ...
    try:
        ...
        logging.error("Error starting ChromeDriver: " + str(e))
        return []

    try:
        logging.info(f"Scraping Google for {search_query}")
